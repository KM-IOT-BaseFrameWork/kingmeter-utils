...
public class CRCUtils {
    ...
    private static int crc_tabccitt[] = {
    ...
    public static synchronized CRCUtils getInstance() {
        if (instance == null) {
    ...
    private int crc_ccitt(int crc, int c) {
        int short_c = 0x00ff & c;
        int tmp = (crc >> 8) ^ short_c;
        crc = ((crc << 8) % 65536) ^ crc_tabccitt[tmp];
        return crc;
    }


    private byte[] create_crc16_high_low(byte[] byte_array, int length) {
        int crc = 0;
        for (int i = 0; i < length; i++) {
            crc = crc_ccitt(crc, byte_array[i]);
        }
    ...
    public byte[] getCheckCrcArray(byte[] bytes) {
        return create_crc16_high_low(bytes,bytes.length);
    }
